    ...
    var music = new Vue({
      ...
      methods: {
        ...
        searchMusic: function () {
          ...
          var that = this
          axios
            .get('https://autumnfish.cn/search?keywords=' + this.queryValue)
            .then(function (response) {
              ...
              for (let i = 0; i < that.musicList.length; i++) {
                ...
                that.musicList[i] = {
                  id: that.musicList[i].id,
                  name: that.musicList[i].name,
                  singerName: singerNameAll,
                  songTime: time,
...
console.log(
  '%c页面加载完毕！消耗了' + Math.round(performance.now() * 100) / 100 + 'ms',
  'background-color:#282c34;color:#51aded'
)
